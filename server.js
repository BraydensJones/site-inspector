...
app.post('/inspect', async (req, res) => {
  ...
  try {
    ...
    // Still nothing -- the form may be behind a button that opens a modal rather than
    // navigating to a new page. Try clicking a matching button/link and rescan in place.
    if (formsProcessed === 0) {
      const clickable = await page.$$eval('button, a, input[type="button"], [role="button"]', (els) =>
        els.map((e, i) => ({ index: i, text: (e.innerText || e.value || '').trim() }))
      ).catch(() => []);
      const target = clickable.find(c => CONTACT_KEYWORDS.test(c.text));
      if (target) {
        const handles = await page.$$('button, a, input[type="button"], [role="button"]');
        const handle = handles[target.index];
        if (handle) {
          await handle.click({ timeout: 3000 }).catch(() => {});
          await page.waitForTimeout(1000);
          result.visited_urls.push(page.url() + ' (after clicking "' + target.text + '")');

          const modalFrameTargets = [page.mainFrame()];
          for (const f of page.frames()) {
            if (f === page.mainFrame()) continue;
            try {
              const fUrl = f.url();
              if (fUrl && /jotform|typeform|calendly|forms\.|form\.|gravityforms|wix|squarespace/i.test(fUrl)) {
                modalFrameTargets.push(f);
              }
            } catch (e) {}
          }
          const pass3 = await runFormPass(modalFrameTargets, formsProcessed);
          result.interactions.push(...pass3.interactions);
          formsProcessed = pass3.formsProcessed;
        }
      }
    }
